...
abstract class LectorXD : KeiSource() {
    ...
    override suspend fun getSearchMangaList(
        ...
        if (query.isBlank()) {
            val document = client.get(
                filteredCatalogueUrl(page, filters, statusQuery),
            ).asJsoup()
    ...
    private suspend fun resolveStatusQuery(
        status: String,
    ): Pair<String, String> {
        statusQueryCache[status]
            ?.let {
                return it
            }

        val candidates =
            STATUS_QUERY_CANDIDATES[
                status
            ].orEmpty()
    ...
    private fun catalogueTotal(
        document: Document,
    ): Int? {
        val raw =
            CATALOGUE_TOTAL_REGEX
                .find(
                    document.text(),
                )
                ?.groupValues
    ...
    private fun Document.toMangasPage(
        page: Int,
    ): MangasPage =
        MangasPage(
            parseCatalogue(this),
    ...
    private fun parseMangaDetails(
        ...
        val title =
            document
                ...
                ?: throw IllegalStateException(
                    "LectorXD: no se pudo encontrar el título",
                )
        ...
        val status =
            when {
                "completado" in pageText ->
                    SManga.COMPLETED

                "en emisión" in pageText ||
                    "en emision" in pageText ->
                    SManga.ONGOING
    ...
    private suspend fun fetchAllChapters(
        ...
        var reachedKnown =
            addFrom(firstDocument)

        val maxPage =
            chapterPaginationMax(
                firstDocument,
            ).coerceAtMost(
                MAX_CHAPTER_PAGES,
            )
    ...
    private fun chapterPaginationMax(
        document: Document,
    ): Int {
        val fromLinks =
            document
    ...
    private fun normalizeSeriesPath(
        path: String,
    ): String? {
        val normalized =
            path
    ...
    private fun Element.bestImageUrl():
        String? {
        val candidates =
            sequenceOf(
                attr("data-src"),
    ...
    private fun Request.withAlternateCdn():
        Request {
        val alternate =
            if (
                url.host ==
    ...
    companion object {
        ...
        private const val CDN_ALPHA =
            "s1.cdnlxd.xyz"

        private const val CDN_BETA =
            "s2.cdnlxd.xyz"

        private val CDN_HOSTS =
            setOf(
                CDN_ALPHA,
                CDN_BETA,
            )

        private const val CATALOGUE_PAGE_SIZE =
            24

        private const val CHAPTERS_PER_PAGE =
            20

        private const val MAX_CHAPTER_PAGES =
            80

        private val SEARCH_PARAMETERS =
            listOf(
                "search",
                "q",
                "query",
        ...
        private val SEARCH_ROUTES =
            listOf(
                "/buscar",
                "/search",
            )

        private val STATUS_QUERY_CANDIDATES =
            mapOf(
                "ongoing" to
                    listOf(
                        "status" to "ongoing",
        ...
        private val CATALOGUE_TOTAL_REGEX =
            Regex(
                "([0-9.,]+)\\s+series disponibles",
                RegexOption.IGNORE_CASE,
            )

        private val SERIES_PATH_REGEX =
            Regex(
                "^/(?:manga|manhwa|manhua|novela|one-shot)/[^/?#]+$",
                RegexOption.IGNORE_CASE,
            )

        private val CHAPTER_PATH_REGEX =
            Regex(
                "^((?:/manga|/manhwa|/manhua|/novela|/one-shot)/[^/?#]+)/leer/[^/?#]+$",
                RegexOption.IGNORE_CASE,
            )
        ...
        private val PAGE_ALT_REGEX =
            Regex(
                "P[aá]gina\\s*\\d+",
                RegexOption.IGNORE_CASE,
            )

        private val PAGINATION_SUMMARY_REGEX =
            Regex(
                "Mostrando\\s+\\d+\\s*[-–]\\s*\\d+\\s+de\\s+(\\d+)|([0-9]+)\\s+cap[ií]tulos",
                RegexOption.IGNORE_CASE,
            )

        private val CDN_IMAGE_REGEX =
            Regex(
                "https://s[12]\\.cdnlxd\\.xyz/[^\\\"'<>\\s]+",
                RegexOption.IGNORE_CASE,
            )
    }
}
